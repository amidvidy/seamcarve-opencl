...
float** MW_ToMatrix(MagickWand *mw_in, int *pH, int *pW) {
    ...
    if (!im_in) { np_free_matrix_float(M); return NULL; }
    
    for (int y = 0 ; y < h ; y++) { // Grab a row at a time, intensity/grayscale float
        mw_ok = ExportImagePixels(im_in, 0, y, w, 1, "I", FloatPixel, M[y], &ex);
        if (mw_ok == MagickFalse) {  np_free_matrix_float(M); return NULL;  }
    }
...
MagickWand* MW_Carve_Grey(const MagickWand *mw_in) {
    ...
    NewMagickWand();  if (!mw_temp) return MagickFalse;
...
Image* IntMatrixToNewImage(int** M, int width, int height) {
    MagickWand *m_Wand = NewImage(width, height, "white");

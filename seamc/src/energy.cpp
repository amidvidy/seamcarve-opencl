		...
		const int width, const int height)
{
	...
	static const
	float kernelWeights[81] = { 0.0f, 1.0f, 1.0f, 2.0f, 2.0f, 2.0f, 1.0f, 1.0f,
			0.0f, 1.0f, 2.0f, 4.0f, 5.0f, 5.0f, 5.0f, 4.0f, 2.0f, 1.0f, 1.0f,
			4.0f, 5.0f, 3.0f, 0.0f, 3.0f, 5.0f, 4.0f, 1.0f, 2.0f, 5.0f, 3.0f,
			-12.0f, -24.0f, -12.0f, 3.0f, 5.0f, 2.0f, 2.0f, 5.0f, 0.0f, -24.0f,
			-40.0f, -24.0f, 0.0f, 5.0f, 2.0f, 2.0f, 5.0f, 3.0f, -12.0f, -24.0f,
			-12.0f, 3.0f, 5.0f, 2.0f, 1.0f, 4.0f, 5.0f, 3.0f, 0.0f, 3.0f, 5.0f,
			4.0f, 1.0f, 1.0f, 2.0f, 4.0f, 5.0f, 5.0f, 5.0f, 4.0f, 2.0f, 1.0f,
		...
		for (int big_x = 0; big_x < width; big_x += block_x) {
			...
			for (int y = big_y; y < end_y; y++) {
				...
				for (int x = big_x; x < end_x; x++) {
					...
					float leftpixel = dot4(luma_coef,
							readImage4Clip(SRC, leftPixelCoord));
					float rightpixel = dot4(luma_coef,
							readImage4Clip(SRC, rightPixelCoord));
					float abovepixel = dot4(luma_coef,
							readImage4Clip(SRC, abovePixelCoord));
					float belowpixel = dot4(luma_coef,
							readImage4Clip(SRC, belowPixelCoord));
					//float gradient = fabs(rightpixel - leftpixel) + fabs(abovepixel - belowpixel);
					// Slightly different formulation of gradient
					float gradient = sqrt(
							pow(rightpixel - leftpixel, 2)
									+ pow(abovepixel - belowpixel, 2));
					gradient *= 1024.0f;
					gradient += 256;

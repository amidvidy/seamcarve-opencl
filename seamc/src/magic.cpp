...
void** MW_ToMatrix(MagickWand *mw_in, int *pH, int *pW, bool isCOLOR)
{
	...
	for (int y = 0; ((mw_ok != MagickFalse) && (y < h)); y++) {
		...
		mw_ok = ExportImagePixels(im_in, 0, y, w, 1, pixMap, FloatPixel, M[y],
				im_ex);
	}
...
MagickWand* MW_Carve(const MagickWand *mw_in, int newH, int newW, bool isCOLOR,
		bool drawLINE)
{
	MagickBooleanType mw_ok;
	...
	mw_temp = MW_FromMatrix(M_out, (drawLINE) ? h : newH, (drawLINE) ? w : newW,
			isCOLOR);

...
void SEAMC_dp(float **Y, float **G, int width, int height)
{
	...
	for (int y = 1; y <= height_m1; y++) {
		...
		pY_y[width_m1] = pG_y[width_m1]
				+ fmin(pY_yp[width_m1], pY_yp[width_m1 - 1]);
	}
} // def dp(Y,G):

void SEAMC_carveKernel(void **DST, void **SRC, int width, int height,
		int32_t *CARVE, int pixBytes)
{
	...
	for (int y = 0; y < height; y++) {
		...
		if (RemainBytes > 0)
			::memmove(dROW + CarveOffset, sROW + CarveOffset + pixBytes,
					RemainBytes);
	}
} // def copyKernel(I,width_m1,c):

void SEAMC_lineKernel(void **DST, void **SRC, int width, int height,
		int32_t *CARVE, int pixBytes)
{
...
void** SEAMC_carve(void **iM, int inW, int inH, int newW, int newH,
...
//TODO: perhaps use the output matrix as the working copy rather than modifying the input matrix.
	...
	void** srcIM = iM;
	void** newM = (void**) np_zero_matrix<float>(fullHeight,
			fullWidth * pixDepth, NULL);
	...
	F4_t** BLUR = (F4_t**) np_zero_matrix<float>(fullHeight,
			fullWidth * pixDepth, NULL);
	...
	while (remainWidth > newW) {   // TODO: Deal with stretch & vertical too!!!
		...
		DebugMatrix((void**) srcIM, WORK.width, WORK.height, "0_start",
				remainWidth, isCOLOR);
		if (isCOLOR) {
			//SEAMC_glaplauxian(O, (const F4_t**) srcIM, WORK.width, WORK.height);
			SEAMC_gaussian(BLUR, (const F4_t**) srcIM, WORK.width, WORK.height);
			DebugMatrix((void**) BLUR, WORK.width, WORK.height, "1_blur",
					remainWidth, true);
			
			SEAMC_gradient(GRAD, const_cast<const F4_t**>(BLUR), WORK.width,
					WORK.height);
		} else {
			if (!KONV) {
			...
			SEAMC_tfj_conv2d(3, 3, WORK.ydim, WORK.xdim, (float**) srcIM, GRAD,
					KONV);
		...
		DebugMatrix((void**) GRAD, WORK.width, WORK.height, "2_grad",
				remainWidth, false);
		
		SEAMC_dp(COST, GRAD, WORK.width, WORK.height);
		DebugMatrix((void**) COST, WORK.width, WORK.height, "3_cost",
				remainWidth, false);
		...
		if (drawLINE) {
			SEAMC_lineKernel(newM, srcIM, WORK.width, WORK.height, CARVE,
